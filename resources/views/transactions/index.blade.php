...
layui.use(['table', 'dropdown'], function(){
  ...
  table.render({
    elem: '#test',
    url: '{{ route("transactions.index") }}', // 此处为静态模拟数据，实际使用时需换成真实接口
    toolbar: '#toolbarDemo',
        ...
        click: function(obj){
          ...
          switch(obj.id){
            ...
            case 'update':
              ...
              window.location.href = '/transactions/' + data[0].id + '/edit';
              break;
            case 'delete':
              if(data.length !== 1){
              ...
              $.ajax({
                    ...
                    success: function(res){
                        if(res.code === 0){
                            ...
                            layui.table.reloadData('test'); // 重载数据
                        } else {
                    ...
                    error: function(xhr){
                        var msg = '删除失败';
                        if(xhr.status === 404) msg = '记录不存在';
                        if(xhr.status === 403) msg = '无权操作';
  ...
  table.on('tool(test)', function(obj){ // 双击 toolDouble
    ...
    // console.log(obj)
    if(obj.event === 'edit'){
      window.location.href = '/transactions/' + data.id + '/edit';
